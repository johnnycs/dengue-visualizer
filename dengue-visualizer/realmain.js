...
d3.json("data/thailand.json", function (json) {
  ...
  // d3.csv("data/newyear_casualties/newyear_casualties_"+select_year+".csv", function(error, data) {
  d3.csv("data/dengue_year/dengue_year_"+select_year+".csv", function(error, data) {
  // d3.csv("public/data/175_samples_all_newyear_casualties.csv", function(error, data) {
    if (error) { // when data is failed to load, do nothing.
      console.error(error);
    } else {
      ...
      .each(function() {
        ...
        var day_captions = ['Jan','Feb','Mar','Apr','May','June','July','Aug','Sept','Oct','Nov','Dec']
